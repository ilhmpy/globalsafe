...
export const Notify: FC<NotifyProps> = ({ block, auth }: NotifyProps) => {
    ...
    return (
      <Notifies.NotifiesBlock block={block} auth={auth} empty={notifies.length === 0}>
            ...
            {notifies && notifies.length ? (
                <>
                    {notifies && notifies.map((notify: any, idx: number) => (
                        <Notifies.Notify checked={notify.view} key={idx}>
